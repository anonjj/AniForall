...
class Animepahe {
    ...
    async refreshCookies() {
        if (this.activeRefreshPromise) return this.activeRefreshPromise;
        ...
        this.activeRefreshPromise = (async () => {
            ...
            try {
                ...
                await page.goto(Config.getUrl('home'), {
                    waitUntil: 'load',  // 'networkidle' times out — CF keeps polling indefinitely
                    timeout: 60000,
                });

                // Give Cloudflare JS challenge time to complete and set its cookies
    ...
    async fetchApiData(endpoint, params = {}, userProvidedCookies = null, _retryAllowed = true) {
        try {
            ...
            // CustomError uses .statusCode; axios errors use .response.status — check both
            const status = error.statusCode ?? error.response?.status;
            const isDdosError = status === 401 || status === 403 || error.message?.includes('DDoS-Guard');
            if (!userProvidedCookies && _retryAllowed && isDdosError) {
                await this.refreshCookies();
                return this.fetchApiData(endpoint, params, userProvidedCookies, false);
            }
    ...
    async scrapeIframeLight(url) {
        // Use axios directly — got-scraping returns a Kwik page variant that
        // doesn't embed the m3u8 in the eval() output, causing extraction to fail.
        // Axios with browser-matching headers gets the real packed script.
        try {
            ...
            if (html.toLowerCase().includes('attention required!')) {
            ...
            console.warn('[scrapeIframeLight] axios fetch failed:', error.message);
            throw error;
        }
    }
