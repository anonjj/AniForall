    ...
    static async extractSources(html, url = '') {
        ...
        for (const script of scriptMatches) {
            ...
            const sandbox = {
                ...
                setTimeout,
                clearTimeout,
                // Base64 — Kwik obfuscated scripts use atob() heavily
                atob: (b64) => Buffer.from(b64, 'base64').toString('binary'),
                btoa: (str) => Buffer.from(str, 'binary').toString('base64'),
                // Intercept fetch() calls that load the m3u8
                fetch: (src) => {
                    try {
                        if (typeof src === 'string' && src.includes('.m3u8')) captured.add(src);
                    } catch (e) {}
                    return Promise.resolve({ ok: true, text: () => Promise.resolve(''), json: () => Promise.resolve({}) });
                },
                // Intercept XMLHttpRequest in case Kwik uses it
                ...
                // Common globals Kwik scripts reference
                JSON,
                Math,
                parseInt,
                parseFloat,
